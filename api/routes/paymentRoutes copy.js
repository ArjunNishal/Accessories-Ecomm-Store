...
const stripe = require("stripe")(
  "sk_lie_51NPirUSGPN3Ia4GzvVadfjMl3iVreKjdBtZ6YIxTmXjqtEpr0h5xZBrSlAqIc4mNVvhg0yO5G89WK1o5tZcoU988vP00OuFV32Nl"
);
// const stripe = require("stripe")("sk_gtest_51NPirUSGPN3Ia4GzghghonG7mYLT5S9wie851xXChWnOpJz4iHBNFhaqjRer9oAN9DOD3LtKSfr8HHyHzkc7sGde4Nae00IQHQBeJN");
...
router.post("/gateway", async (req, res) => {
  // const orderData = req.body; // Retrieve payment data from the frontend
  const { getdata, orderId } = req.body;
  var db = JSON.stringify(orderId);
  var orderid = JSON.parse(db);
  const discountAmt = await Order.findById(orderid);
  const TotalAmount = discountAmt.ordertotal;
  // encRequest = ccav.encrypt(discountAmt, encryptionKey);

  // Generate a secure checksum based on orderData
  const secureHash = crypto
    .createHash("sha256")
    .update(discountAmt + encryptionKey)
    .digest("hex");
  console.log(secureHash);
  const success = `${constants.frontUrl}order/success?order_id=${orderId}`;
  const failure = `${constants.frontUrl}cart`;
  // Redirect the user to CCAvenue's payment page with the necessary parameters
  const redirectUrl = `https://secure.ccavenue.com/transaction/transaction.do?command=initiateTransaction&merchant_id=${merchantId}&access_code=${accessCode}&order_id=${discountAmt.orderId}&amount=${discountAmt.amount}&currency=INR&redirect_url=${success}&cancel_url=${failure}&language=EN&billing_name=${discountAmt.name}&billing_address=${discountAmt.address.address1}&checksum=${secureHash}`;
