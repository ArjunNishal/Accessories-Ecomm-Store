  ...
  const handleDeleteCharm = (charmId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosInstance
          .delete(`charm/delete/${charmId}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
          .then((response) => {
            Swal.fire("Deleted!", "Charm has been deleted.", "success");
            fetchCharms();
          })
          .catch((error) => {
            console.error(error);
          ...
          <div className="charmpage">
            <div className="d-flex justify-content-between align-items-center">
              <h1 className="user-h1">
                <i class="fa-solid fa-clover"></i> Charms
              </h1>
              <div>
                {showaddcharm ? (
                  <button
            ...
            {showaddcharm ? (
              <div className="container">
                <CharmForm charmId={charmid} fetchCharms={fetchCharms} />
              </div>
            ) : (
