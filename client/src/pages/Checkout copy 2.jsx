...
import { CheckoutFunc } from "./CheckoutFunc";
...
const Checkout = () => {
  ...
  console.log(formData, appliedcoupon, "formdata");
  ...
  const handleplaceorder = async (event) => {
    ...
    try {
      const itemsJson = JSON.stringify(getdata);
      const formdata = { items: itemsJson };
      const formDataParams = new URLSearchParams(formdata).toString();
      localStorage.setItem("formData", JSON.stringify(formData));

      // navigate(`/payment?${formDataParams}`);
      CheckoutFunc({formDataParams});

      // Navigate to the payment page with the formDataParams as URL parameters
    } catch (error) {
